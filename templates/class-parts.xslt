...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://sf.net/saxon" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ifn="urn:internal:functions" xmlns:fn="http://www.w3.org/2005/xpath-functions"
	...
	<xsl:template name="classHeader">
		...
		<div class="MainContent">
			...
			<table class="classHeaderTable" cellpadding="0" cellspacing="0" width="100%">
				<tr>
					<td class="classHeaderTableLabel" width="80">
						<xsl:choose>
...
<xsl:if test="$config/linkReport/show = 'true'">
					...
					<xsl:variable name="nodes">
						<xsl:apply-templates select="$linkReportNode">
							<xsl:sort select="def/@id" data-type="text" order="ascending" />
							<xsl:with-param name="skip" select="$linkReportNode" />
						</xsl:apply-templates>
					</xsl:variable>
					
					<td rowspan="32" width="80">
						<table width="100%" height="80" cellpadding="0" cellspacing="0" class="costTable">
							<tr height="100%">
								<td class="costArea">
									<span class="costSize">
				...
				<xsl:choose>
					<xsl:when test="$testNode">
						...
						<a href="{$config/tests/link}/{replace($testSuite/@package,'\.','/')}/{$testSuite/@id}_{$testSuite/@name}.html">
							...
							test<xsl:if test="$testNode//@tests > 1">s</xsl:if> 
						</a>
						<xsl:if test="$coverageNode">
							providing <xsl:value-of select="round($coverageNode/@branchCoverage * 100)" />% branch coverage.
						</xsl:if>
					</xsl:when>
					<xsl:otherwise>
...
<!-- end modification -->
	...
	<!-- customized by Ben Stucki -->
	<!-- this is for calculating unit cost of classes from link-reports -->
	
	<!-- calls recursive function and cleans dupicates -->
	
	
	<!-- recursive function to pull all references from scripts -->
	<xsl:template match="script">
		<xsl:param name="skip" />
		
		<xsl:variable name="references" select="pre[not(@id = $skip/def/@id)] | dep[not(@id = $skip/def/@id)]" />
		<xsl:variable name="workscripts" select="//report/scripts/script[def/@id=$references/@id]" />
		
		<xsl:variable name="nodes">
			<script def_id="{def/@id}" references="{count($references)}" skip="{count($skip)}" optimizedsize="{@optimizedsize}">
				<def id="{def/@id}" />
			</script>
			<xsl:for-each select="$references">
				<xsl:variable name="id" select="@id" />
				<xsl:variable name="script" select="//report/scripts/script[def/@id=$id]" />
				<xsl:if test="count($script) > 0" >
					<xsl:apply-templates select="//report/scripts/script[def/@id=$id]">
						<xsl:sort select="def/@id" data-type="text" order="ascending" />
						<xsl:with-param name="skip" select="$skip | $workscripts" />
					</xsl:apply-templates>
				</xsl:if>
			</xsl:for-each>
		</xsl:variable>
		...
		<!-- remove duplicates -->
		<!-- (... because I couldn't find a way to += skip in the above recursion) -->
		<xsl:for-each select="exsl:node-set($nodes)/script[not(def/@id=preceding-sibling::node()/def/@id)]">
			<xsl:sort select="def/@id" />
				<xsl:copy-of select="." />
		</xsl:for-each>
